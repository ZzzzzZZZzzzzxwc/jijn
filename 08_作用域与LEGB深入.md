...
>
> 📚 **基础回顾**：本章基于 [§7.2 名字与对象的关系](07_引用与内存机制.md#72-名字与对象的关系)。  
> 在 7.2 中我们建立了「**名字 = 标签，对象 = 真实数据**」的模型；本章要回答的是：**当你写下一个名字时，Python 究竟去哪些"标签集合"里找它？** 那些"标签集合"就是命名空间，而 LEGB 就是查找顺序。如果 7.2 不熟，强烈建议先回去过一遍——下面所有内容都依赖那个心智模型。
