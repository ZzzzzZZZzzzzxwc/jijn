...
> ⚠️ **进阶补充**：即使是 C 实现的方法（如 `list.sort()`），如果排序过程中调用了 Python 层的 `__lt__`，GIL 也会在每次 `__lt__` 调用的字节码边界释放。所以"调用 C 方法期间不会切换线程"这个说法**不成立**——只有不回调 Python 代码的纯 C 操作（如 `time.sleep`、`socket.recv`）才在整个调用期间持有/释放 GIL。
